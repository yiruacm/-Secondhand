	...
	*/
	public function save(){
		...
		if (!empty($_FILES["file3"]["tmp_name"])) {
			//文件上传
			$info3 = $this->upload_pic($_FILES["file3"],array('jpg','png','jpeg'),"category/".date(Ymd));
		    if(!is_array($info3)) {// 上传错误提示错误信息
		        $this->error($info3);
		        exit();
		    }else{// 上传成功 获取上传文件信息
				$this->category->bz_2 = 'UploadFiles/'.$info3['savepath'].$info3['savename'];
			    if (intval($_POST['cid'])) {
					$photo = $this->category->where('id='.intval($_POST['cid']))->getField('bz_2');
				}
		    }
		}
		//保存数据
